...
--exec sp_EmpaqueMasterIBM'93183','S 2210','4997' 
alter PROCEDURE sp_EmpaqueMasterIBM
@numlot numeric,  
...
exec fun_retornauuid @numlot,@producto ,@embfactura,'2019','MST',null,@uuid out
...
--SELECT convert(numeric(18,2),SUM(tcd_cantid/emb_cantid)) MASTEER,'EA' med_abrev,clp_codigo
...
INNER JOIN tb_medida ON med_codigo= PRO_UNIMED
inner join tb_productosIBM on pro_codcor = pri_codcor
where lid_lote = @numlot and lid_produc = @producto and   liq_estado <> 'AN'  and liq_tipo <> 'RE'
group by med_abrev,med_factor,emb_peso,clp_codigo,pri_cadenaUrn+pri_prefijoSonga+'.'+pri_prefix_master+'.'+convert(varchar(20),lid_lote)
...
inner join tb_detproces on dpr_codigo = pro_congela
 ...
 begin
	set @fecha = (select top 1  trc_fecha
	from tb_tracadauto
...
--select * from tb_tracadauto
--inner join tb_tracamauto on trc_numsec = tcd_numero
--inner join tb_bodega on bod_codigo= trc_codcam and bod_categ ='tu'
--where tcd_lote='92025' and tcd_produc ='5009' and trc_ingegr='T'
--order by trc_fecha
...
,'urn:epcglobal:cbv:bizstep:EmpaqueMaster'  bizStep
into #TMP;
...
FROM
(
      select (SELECT eventTime,eventTimeZoneOffset,eventID,id,bizStep, [source/@type],[source],[destination/@type],destination,
	   (select   Detalle epcClass,LIBRAS quantity, med_abrev uom from #TMPDATOS for xml path, type) as inputQuantityList,
	   (select  Detalle epcClass,MASTEER quantity, med_abrev uom from #TMPDATOS2 for xml path, type) as outputQuantityList
                  FROM #TMP D
